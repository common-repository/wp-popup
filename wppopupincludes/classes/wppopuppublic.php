...
if(!class_exists('wppopuppublic')) {
	...
	class wppopuppublic {
					...
					if(!empty($wppopup_check)) {
						...
						foreach($order as $key) {
							...
							switch ($key) {
								...
								case 'incountry':	$incountry = $this->incountry( $wppopup_incountry );
								...
								case 'notincountry':
													$incountry = $this->incountry( $wppopup_notincountry );
													if($incountry || $incountry === 'XX') {
														$show = false;
													}
													break;
		...
		function get_country_from_cache( $ip ) {
			...
			$country = $this->db->get_var( $this->db->prepare( "SELECT country FROM {$this->wppopup_ip_cache} WHERE IP = %s", $ip ) );
		...
		function put_country_in_cache( $ip, $country ) {
			...
			return $this->insertonduplicate( $this->wppopup_ip_cache, array( 'IP' => $ip, 'country' => $country, 'cached' => time() ) );
