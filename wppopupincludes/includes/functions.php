<?php
function set_wppopup_url($base) {

	global $wppopup_url;

	if(defined('WPPP_PLUGIN_URL') && defined('WPPP_PLUGIN_DIR') && file_exists(WPPP_PLUGIN_DIR . '/' . basename($base))) {
		$wppopup_url = trailingslashit(WPPP_PLUGIN_URL);
	} elseif(defined('WP_PLUGIN_URL') && defined('WP_PLUGIN_DIR') && file_exists(WP_PLUGIN_DIR . '/wp-popup/' . basename($base))) {
		$wppopup_url = trailingslashit(WP_PLUGIN_URL . '/wp-popup');
	} else {
		$wppopup_url = trailingslashit(WP_PLUGIN_URL . '/wp-popup');
	}
...
function set_wppopup_dir($base) {

	global $wppopup_dir;

	if(defined('WPPP_PLUGIN_DIR') && file_exists(WPPP_PLUGIN_DIR . '/' . basename($base))) {
		$wppopup_dir = trailingslashit(WPPP_PLUGIN_URL);
	} elseif(defined('WP_PLUGIN_DIR') && file_exists(WP_PLUGIN_DIR . '/wp-popup/' . basename($base))) {
		$wppopup_dir = trailingslashit(WP_PLUGIN_DIR . '/wp-popup');
	} else {
		$wppopup_dir = trailingslashit(WP_PLUGIN_DIR . '/wp-popup');
	}
...
function wppopup_url($extended) {

	global $wppopup_url;

	return $wppopup_url . $extended;

}

function wppopup_dir($extended) {

	global $wppopup_dir;

	return $wppopup_dir . $extended;


}

function wppopup_helpimage( $image ) {
	echo "<img src='" . wppopup_url('wppopupincludes/help/images/' . $image) . "' />";
}

function wppopup_db_prefix(&$wpdb, $table) {
...
function get_wppopup_addons() {
	if ( is_dir( wppopup_dir('wppopupincludes/addons') ) ) {
		if ( $dh = opendir( wppopup_dir('wppopupincludes/addons') ) ) {
			$pop_addons = array ();
			...
			return apply_filters('wppopup_available_addons', $pop_addons);
...
function load_wppopup_addons() {

	$addons = get_option('wppopup_activated_addons', array());
	...
	if ( is_dir( wppopup_dir('wppopupincludes/addons') ) ) {
		if ( $dh = opendir( wppopup_dir('wppopupincludes/addons') ) ) {
			...
			$pop_addons = apply_filters('wppopup_available_addons', $pop_addons);

			foreach( $pop_addons as $pop_addon ) {
				if(in_array($pop_addon, (array) $addons)) {
					include_once( wppopup_dir('wppopupincludes/addons/' . $pop_addon) );
				}
			}
...
function load_all_wppopup_addons() {
	if ( is_dir( wppopup_dir('wppopupincludes/addons') ) ) {
		if ( $dh = opendir( wppopup_dir('wppopupincludes/addons') ) ) {
			$pop_addons = array ();
			while ( ( $addon = readdir( $dh ) ) !== false )
...
function P_style_urls( $styles = array() ) {
	...
	$styles['Default'] = wppopup_url('wppopupincludes/css/default');
	$styles['Default Fixed'] = wppopup_url('wppopupincludes/css/fixed');
	$styles['Dark Background Fixed'] = wppopup_url('wppopupincludes/css/fullbackground');

	return $styles;
}
add_filter( 'wppopup_available_styles_url', 'P_style_urls');

function P_style_dirs() {
	$styles['Default'] = wppopup_dir('wppopupincludes/css/default');
	$styles['Default Fixed'] = wppopup_dir('wppopupincludes/css/fixed');
	$styles['Dark Background Fixed'] = wppopup_dir('wppopupincludes/css/fullbackground');

	return $styles;
}
add_filter( 'wppopup_available_styles_directory', 'P_style_dirs');
...
function P_CountryList() {
	...
	return apply_filters( 'wppopup_country_list', $countries );
