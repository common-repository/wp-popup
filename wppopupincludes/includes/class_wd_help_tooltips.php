 ...
 * Example usage:
 * <code>
 * if (!class_exists('WPPPDev_HelpTooltips')) require_once YOUR_PLUGIN_BASE_DIR . '/lib/external/class_wd_help_tooltips.php';
 * $tips = new WPPPDev_HelpTooltips();
 ...
 * $tips1 = new WPPPDev_HelpTooltips();
 ...
 * $tips2 = new WPPPDev_HelpTooltips();
...
if(!class_exists('WPPPDev_HelpTooltips')) {
class WPPPDev_HelpTooltips {
	...
	public function bind_tip  ($tip, $bind_to_selector) {
		$tip_id = 'WPPPdev-help-tip-for-' . md5($bind_to_selector);
	...
	private function _get_tip_markup ($tip, $arg='') {
		return "<span class='WPPPdev-help' {$arg}>{$tip}</span>";
	}
	...
	private function _print_styles () {
		...
		if (!defined('WPPPDEV_TOOLTIPS_CSS_ADDED')) define('WPPPDEV_TOOLTIPS_CSS_ADDED', true);
		else return false;
...
.WPPPdev-help-trigger span {
	display: block;
	position: absolute;
	left: -12000000px;
}
.WPPPdev-help-trigger {
	position: relative;
...
.WPPPdev-right_pointer {
	...
	private function _print_scripts () {
		// Have we already done this?
		if (!defined('WPPPDEV_TOOLTIPS_JS_ADDED')) define('WPPPDEV_TOOLTIPS_JS_ADDED', true);
		else return false;
...
function initialize_help_item ($me) {
	var $prev = $me.prev();
	var help = '&nbsp;<a class="WPPPdev-help-trigger" href="#help"><span><?php _e('Help');?></span></a>';
	$prev = $prev.length ?
		$prev.after(help)
		:
		$me.before(help)
...
function get_help_block ($me) {
	...
	return $parent.find('.WPPPdev-help');
}
...
function show_help_block ($me) {
	...
	if ($("#WPPPdev-tooltip").length) $("#WPPPdev-tooltip").remove();
...
function open_tooltip ($me) {
	...
	if ($("#WPPPdev-tooltip").length) $("#WPPPdev-tooltip").remove();
	if (!$("#WPPPdev-tooltip").length) $("body").append('<div id="WPPPdev-tooltip"><div class="WPPPdev-pointer WPPPdev-left_pointer"></div><div id="WPPPdev-tooltip-source"></div></div>');
	var $tip = $("#WPPPdev-tooltip");
	...
	var $pointer = $tip.find(".WPPPdev-pointer");
...
<?php if (!is_rtl()) { ?>
	if ((left_pos+width+60) >= $(window).width()) {
		...
		$pointer
			.removeClass("WPPPdev-left_pointer")
			.addClass("WPPPdev-right_pointer")
		;
	}
<?php } else { ?>
...
<?php } ?>
	...
	// IE safeguard
	if ($.browser.msie) {
		var $pointer_left = $tip.find(".WPPPdev-left_pointer");
		if ($pointer_left.length) $pointer_left.css("position", "absolute");
	}

	$tip
		// Populate tip text
		.find("#WPPPdev-tooltip-source")
			.width(width)
			.html($help.html())
		.end()
		// Position tip
...
function close_tooltip () {
	if (!$("#WPPPdev-tooltip").length) return false;

	// IE conditional alternate removal
	if ($.browser.msie) {
		$("#WPPPdev-tooltip").hide('fast');
	} else {
